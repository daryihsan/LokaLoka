    ...
    <div class="flex items-center justify-between max-w-7xl mx-auto">
        ...
        <div class="flex items-center gap-3">
            ...
            transition" aria-label="Loka Loka ke Beranda">
                {{-- Set background none pada gambar dan pakai ukuran yang lebih kecil --}}
                <img src="{{ asset('images/logo.png') }}" alt="Logo Loka Loka"
                    class="w-8 h-auto object-contain flex-shrink-0" style="background: none; mix-blend-mode: multiply;">
                Loka Loka
            </a>
        </div>
        ...
        @if (request()->routeIs('homepage') || request()->routeIs('searchfilter'))
            <form method="GET" action="{{ route('searchfilter') }}" class="flex-1 mx-8 max-w-2xl">
                <div class="relative">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari produk lokal terbaik..."
                    ...
                    <button type="submit" class="absolute left-3 top-3.5 h-5 w-5 text-green-dark"
                        style="background: none; border: none; padding: 0;" aria-label="Cari">
        ...
        <div class="flex gap-3">
            {{-- Menggunakan Session::has('user_id') sebagai indikator login yang lebih kuat --}}
            @if(session()->has('user_id'))
                ...
                <a href="{{ route('cart.show') }}" class="text-white hover:bg-white
                    ...
                    <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L6 18h12M6 18a2 2 0 100 4 2 2 0 000-4zM16 18a2 2 0 100 4 2 2 0 000-4z">
                        </path>
                    </svg>
                ...
                @if (request()->routeIs('homepage') || request()->routeIs('searchfilter') || request()->routeIs('product.show'))
                    <div class="relative">
                        <button type="button"
                            ...
                            onclick="toggleUserMenu()" aria-haspopup="true" aria-expanded="false" aria-controls="user-dropdown"
                            id="user-menu-button">
                            <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
...
{{-- Ambil error yang di-pass dari Controller --}}
@if ($errors->any())
...
<script>
    ...
    function showToast(type, message) {
        ...
        setTimeout(() => {
            ...
            // 3. Matikan interaksi HANYA setelah transisi KELUAR (350ms) selesai.
