                                ...
                                <div class="relative">
                                    <div class="h-32 w-full rounded-t-2xl relative overflow-hidden bg-gray-100">
                                        ...
                                        @if($product->discount_percent ?? null)
                                            <div
                                                class="absolute top-2 left-2 bg-red-600 text-white px-2 py-1 rounded-full text-xs font-semibold">
                                                -{{ $product->discount_percent }}%</div>
                                        @endif
                                    </div>
                                    <div class="p-4">
                                        <h3 class="font-open-sans font-semibold text-green-darker mb-2 line-clamp-2">
                                            {{ $product->name }}</h3>
                                        ...
                                        <div class="flex items-center gap-1 mb-2">
                                            ...
                                            <span
                                                class="text-xs text-gray-600">{{ number_format($product->rating ?? 4.5, 1) }}</span>
                                            <span class="text-xs text-gray-500">• Terjual
                                                {{ number_format($product->total_sold ?? 0) }}+</span>
                                        </div>

                                        <div class="flex items-center justify-between">
                                            <div>
                                                <p class="font-bold text-green-800">Rp
                                                    {{ number_format($product->price, 0, ',', '.') }}</p>
                                            </div>
                                            <button onclick="addToCart(event, {{ $product->id }})"
                                                class="bg-primary hover:bg-primary-dark text-white rounded-full p-2"
                                                title="Tambah ke keranjang">
                                                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                        d="M3 3h2l.4 2M7 13h10l4-8H5.4m.6 8L6 18h12M6 18a2 2 0 100 4 2 2 0 000-4zM16 18a2 2 0 100 4 2 2 0 000-4z">
                                                    </path>
                                                </svg>
                                            </button>
                                        </div>
                ...
                @forelse ($categories as $i => $category)
                    <a href="{{ route('searchfilter', ['category' => $category->id]) }}" class="block">
                        <div class="flex flex-col items-center justify-center bg-white rounded-2xl p-6 border-2 shadow-md hover:shadow-xl hover:scale-105 transition-all duration-300 cursor-pointer group opacity-0 animate-fade-in-up"
                            ...
                            <p class="font-open-sans text-sm font-semibold text-green-darker text-center">{{ $category->name }}
                            </p>
                        </div>
                    </a>
                @empty
                                ...
                                <div class="relative">
                                    ...
                                    <div class="p-4">
                                        ...
                                        <div class="flex items-center gap-1 mb-2">
                                            ...
                                            <span
                                                class="text-xs text-gray-600">{{ number_format($product->rating ?? 4.0, 1) }}</span>
...
@push('scripts')
    <script>
        ...
        function addToCart(event, productId) {
            ...
            // Gunakan Session::has('logged_in') yang dicetak ke JS
        ...
        function showToast(type, message) {
            // Menggunakan wadah notifikasi global yang baru
