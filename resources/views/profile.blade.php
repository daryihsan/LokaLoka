...
@section('content')
    ...
    <section class="main-content grid md:grid-cols-2 gap-6">
        <div class="card p-6">
            ...
            <div class="flex flex-col items-center mb-4">
                ...
                @endphp
                <img src="{{ $user_avatar }}" id="user-avatar-display" alt="Avatar Pengguna"
                    class="w-24 h-24 rounded-full object-cover mb-3 border-4 border-primary shadow-lg">
            ...
            <div class="identity-info space-y-2">
                ...
                <p><strong>Bergabung sejak:</strong> {{ $user->created_at ? $user->created_at->format('d F Y') : 'N/A' }}
                </p>
                <p><strong>Update terakhir:</strong> {{ $user->updated_at ? $user->updated_at->format('d F Y') : 'N/A' }}
                </p>
            </div>
        </div>

        <div class="card p-6 lg:col-span-1">
            {{-- Tombol Kembali dengan history.back() atau ke homepage --}}
            <button class="btn btn-secondary mb-4" onclick="window.location.href = '{{ route('homepage') }}'">
                Kembali
            </button>
            ...
            <div class="filter-controls flex justify-between items-center mb-4 relative">
                ...
                <div id="filter-options"
                    class="hidden absolute top-full left-0 mt-2 w-40 bg-white border border-gray-200 rounded-lg shadow-xl z-10">
                    <a href="#" data-status="semua" class="block px-4 py-2 hover:bg-gray-100 rounded-lg text-sm">Semua
                        Status</a>
                    <a href="#" data-status="diproses"
                        class="block px-4 py-2 hover:bg-gray-100 rounded-lg text-sm">Diproses</a>
                    <a href="#" data-status="dikirim"
                        class="block px-4 py-2 hover:bg-gray-100 rounded-lg text-sm">Dikirim</a>
                    <a href="#" data-status="selesai"
                        class="block px-4 py-2 hover:bg-gray-100 rounded-lg text-sm">Selesai</a>
                    <a href="#" data-status="dibatalkan"
                        class="block px-4 py-2 hover:bg-gray-100 rounded-lg text-sm">Dibatalkan</a>
                </div>
            </div>
            <div class="order-list divide-y">
                <div id="no-orders-message" class="text-center text-gray-500 py-4 hidden">Tidak ada pesanan dengan status
                    ini.</div>

                @if(isset($orders) && count($orders) > 0)
                    {{-- Loop menggunakan data $orders dari Controller (dengan relasi OrderItems & Product) --}}
                    @foreach($orders as $order)
                        ...
                        @endphp
                        <a href="{{ route('payment.other', $order->id) }}"
                            class="block order-item hover:bg-gray-50 transition duration-150" data-status="{{ $status }}"
                ...
                <div class="p-6">
                    <form id="editProfileForm" method="POST" action="{{ route('profile.update') }}" class="space-y-4">
                        ...
                        <div>
                            ...
                            <input class="mt-1 w-full border border-gray-300 rounded-lg p-2" type="text" name="name"
                        ...
                        <div>
                            ...
                            <input class="mt-1 w-full border border-gray-300 rounded-lg p-2" type="email" name="email"
                        ...
                        <div>
                            ...
                            <input class="mt-1 w-full border border-gray-300 rounded-lg p-2" type="text" name="phone_number"
                        ...
                        <div class="border-t pt-4">
                            ...
                            <div>
                                ...
                                <input class="mt-1 w-full border border-gray-300 rounded-lg p-2" type="password"
    ...
    <div id="avatarModal" class="fixed inset-0 z-50 hidden">
        <div class="absolute inset-0 bg-black bg-opacity-50" onclick="closeAvatarModal()" aria-hidden="true"></div>
        <div class="relative z-10 flex items-center justify-center min-h-screen p-4">
                        ...
                        @foreach($avatars as $avatar)
                            <label class="cursor-pointer">
                                ...
                                <img src="{{ $avatar }}" alt="Avatar"
                                    class="w-full h-auto rounded-full object-cover border-4 border-transparent hover:border-primary transition-colors duration-200">
                            </label>
                        @endforeach
                    </div>
