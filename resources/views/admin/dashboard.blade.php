...
<body class="bg-gray-100 flex h-screen">
    ...
    <aside class="sidebar w-64 p-6 flex flex-col fixed h-full shadow-2xl">
        <div class="mb-10">
            <a href="{{ route('admin.dashboard') }}"
                class="flex items-center gap-2 text-3xl font-bold text-white hover:text-green-lime transition duration-150">
        ...
        <div class="mt-auto">
            <form action="/logout" method="POST">
                @csrf
                <button type="submit"
                    class="w-full text-left px-4 py-2 rounded-lg text-sm text-red-400 hover:bg-gray-700 hover:text-red-300 transition duration-150">
                    Logout
                </button>
            </form>
        </div>
    ...
    <main class="flex-1 p-8 ml-64 overflow-y-auto">
        ...
        <section id="overview-section" class="space-y-6" @if($section !== 'overview') style="display: none;" @endif>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-md p-6 border border-gray-200">
                    <h3 class="text-xl font-semibold text-gray-800 mb-4">Pengguna Baru Belum Disetujui
                        ({{ $stats['unapproved_users'] }})</h3>
                    <div class="space-y-3">
                        @php
                            // Ambil 5 user 'pending' terbaru untuk ditampilkan
                        ...
                        @forelse ($pendingUsers as $user)
                            <div class="flex justify-between items-center border-b pb-2">
                                ...
                                <form action="{{ route('admin.users.approve', $user->id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit"
                                        class="text-xs bg-green-500 hover:bg-green-600 text-white font-bold py-1 px-3 rounded transition">
                                        Approve
                                    </button>
                                </form>
                            </div>
                        ...
                        @if ($stats['unapproved_users'] > 5)
                            <div class="text-center pt-2">
                                <a href="{{ route('admin.dashboard', ['section' => 'users']) }}"
                                    class="text-sm text-blue-500 hover:text-blue-700 font-medium">Lihat Semua
                                    ({{ $stats['unapproved_users'] }} total) &rarr;</a>
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <tbody id="products-table" class="bg-white divide-y divide-gray-200">
                        @forelse ($products as $product)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center">
                                        <img src="{{ $product->image_url }}"
                                            ...
                                            alt="{{ $product->name }}" class="w-10 h-10 object-cover rounded-lg mr-3">
                                ...
                                <td
                                    class="px-6 py-4 whitespace-nowrap text-sm {{ $product->stock < 5 ? 'text-red-500 font-semibold' : 'text-gray-900' }}">
                                    {{ $product->stock }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="editProduct({{ json_encode($product) }})"
                                        class="text-blue-600 hover:text-blue-900 transition mr-2">Edit</button>
                                    <button onclick="deleteAction('product', '{{ $product->id }}')"
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <button onclick="editCategory({{ json_encode($category) }})"
                                        class="text-blue-600 hover:text-blue-900 transition mr-2">Edit</button>
                                    <button onclick="deleteAction('category', '{{ $category->id }}')"
                                        class="text-red-600 hover:text-red-900 transition">Hapus</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="4" class="px-6 py-4 text-center text-gray-500">Tidak ada kategori ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    <tbody id="orders-table" class="bg-white divide-y divide-gray-200">
                        @forelse ($orders as $order)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="status-badge status-{{ str_replace(' ', '-', strtolower($order->status)) }}">
                                        {{ $order->status }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-6 py-4 text-center text-gray-500">Tidak ada pesanan ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
                    ...
                    <tbody id="users-table" class="bg-white divide-y divide-gray-200">
                        @forelse ($users as $user)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium capitalize">{{ $user->role }}
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    @if($user->approved == 0)
                                        <button onclick="approveAction('{{ $user->id }}')"
                                            class="text-green-600 hover:text-green-900 transition mr-2">Setujui</button>
                                    @else
                                        <button onclick="editUser({{ json_encode($user) }})"
                                            class="text-blue-600 hover:text-blue-900 transition mr-2">Edit</button>
                                    @endif
                                    <button onclick="deleteAction('user', '{{ $user->id }}')"
                                        class="text-red-600 hover:text-red-900 transition">Hapus</button>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-gray-500">Tidak ada pengguna ditemukan.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    <div id="product-modal"
        class="modal fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center opacity-0 pointer-events-none z-50">
        <div
            class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 transform transition-transform duration-300 scale-90">
            ...
            <form id="product-form" method="POST" action="{{ route('admin.products.store') }}">
                ...
                <div class="mb-4">
                    ...
                    <select id="product-category_id" name="category_id" required
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
                ...
                <div class="mb-4">
                    <label for="product-image_url" class="block text-sm font-medium text-gray-700">URL Gambar (Online
                        Link)</label>
                    <input type="url" id="product-image_url" name="image_url"
                        class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2">
                </div>
                <div class="mb-4">
                ...
                <div id="product-active-container" class="mb-4 hidden">
                    <input type="checkbox" id="product-is_active" name="is_active" value="1"
                        class="rounded border-gray-300 text-green-darker shadow-sm focus:border-green-darker focus:ring focus:ring-green-darker focus:ring-opacity-50">
                ...
                <div class="flex justify-end space-x-3">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-green-700 text-white rounded-md hover:bg-green-800">Simpan</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div id="category-modal"
        class="modal fixed inset-0 bg-gray-600 bg-opacity-75 flex items-center justify-center opacity-0 pointer-events-none z-50">
        <div
            class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 transform transition-transform duration-300 scale-90">
            ...
            <form id="user-form" method="POST" action="">
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="closeModal('user-modal')"
                        class="px-4 py-2 border rounded-md text-gray-700 hover:bg-gray-100">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-green-700 text-white rounded-md hover:bg-green-800">Simpan
                        Perubahan</button>
                </div>
            </form>
        </div>
