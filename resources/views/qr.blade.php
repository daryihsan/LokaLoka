        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="bg-white rounded-2xl shadow-xl p-8 text-center">
                ...
                <div class="flex items-center justify-center mb-6">
                    <div class="relative">
                        <svg class="w-16 h-16 transform -rotate-90" viewBox="0 0 80 80">
                            ...
                            <circle cx="40" cy="40" r="36" stroke="#3b82f6" stroke-width="8" fill="none" class="timer-ring">
                            </circle>
                        </svg>
                ...
                <div id="payment-status" class="text-center">
                    <div
                        class="pulse inline-flex items-center px-4 py-2 bg-yellow-100 text-yellow-800 rounded-full text-sm font-medium">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8">
                <h3 class="font-roboto text-xl font-bold text-green-darker mb-6">Ringkasan Pesanan
                    ({{ str_pad($order->id, 6, '0', STR_PAD_LEFT) }})</h3>

                <div class="space-y-4 mb-6 max-h-60 overflow-y-auto pr-2">
                    @foreach($order->orderItems as $item)
                        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-lg">
                            <div
                                class="w-12 h-12 bg-gray-200 rounded-lg flex items-center justify-center relative overflow-hidden">
                                {{-- Link ke Detail Produk dan Gambar --}}
                            ...
                            <div class="flex-1 min-w-0">
                                <h4 class="font-medium text-green-darker">
                                    <a href="{{ route('product.show', $item->product->id) }}"
                                        class="hover:underline">{{ $item->product->name ?? 'Produk Dihapus' }}</a>
                                    <p class="text-sm text-gray-600">{{ $item->qty }}x
                                        @php echo number_format($item->price, 0, ',', '.') @endphp</p>
                            </div>
                ...
                <div class="mt-6 space-y-3">
                    <a href="{{ route('profile') }}"
                        class="block w-full text-center bg-gray-200 hover:bg-gray-300 text-gray-800 py-3 rounded-lg font-medium transition-colors">Lihat
                        Pesanan Saya</a>
                </div>
            </div>
        </div>
